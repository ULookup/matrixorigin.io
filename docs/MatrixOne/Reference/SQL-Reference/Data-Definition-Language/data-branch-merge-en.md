...
The system automatically identifies the Lowest Common Ancestor (LCA) between two tables and calculates the changes that need to be merged based on it. Since v3.0.12, the LCA resolution and collect-range computation work across branches of **arbitrary DAG depth**, not just direct parent-child relationships. When two branches make different modifications to the same row of data, a conflict occurs, which can be handled using conflict handling options.
